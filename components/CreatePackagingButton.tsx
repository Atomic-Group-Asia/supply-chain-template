...
export function CreatePackagingButton({ suppliers }: { suppliers: any[] }) {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className="bg-[#1A1A1A] text-[#FAFAF7] px-3.5 py-2 rounded text-[13px] hover:bg-[#C8432C] transition-colors">+ New Packaging</button>
        ...
        >
          ...
          >
            <div style={{ padding: '20px 40px', borderBottom: '1px solid #D4D0C7', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              <div>
                <div style={{ ...labelStyle, marginBottom: '2px' }}>New packaging</div>
                <h2 style={{ fontSize: '22px', fontWeight: 500, margin: 0 }}>Create packaging</h2>
              </div>
            ...
            <div style={{ padding: '24px 40px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', flex: 1 }}>
              ...
              <Field label="Code * (manual, e.g. N-SBOX-15S)">
                <input value={form.packaging_code} onChange={e => update('packaging_code', e.target.value.toUpperCase())} style={inputStyle} placeholder="UPPERCASE-HYPHENATED" />
              </Field>
              ...
              <Field label="Type">
                <input value={form.packaging_type} onChange={e => update('packaging_type', e.target.value)} style={inputStyle} placeholder="Box / Bottle / Foil / Label / Sachet / Others" list="pkg-types-c" />
                <datalist id="pkg-types-c">
            ...
            <div style={{ padding: '16px 40px', borderTop: '1px solid #D4D0C7', display: 'flex', justifyContent: 'flex-end', gap: '8px', flexShrink: 0 }}>
              ...
              <button onClick={submit} disabled={submitting || !form.packaging_code || !form.packaging_name || !form.brand} style={{ padding: '8px 18px', borderRadius: '4px', fontSize: '13px', backgroundColor: '#1A1A1A', color: '#FAFAF7', border: 'none', cursor: 'pointer', opacity: (submitting || !form.packaging_code || !form.packaging_name || !form.brand) ? 0.5 : 1 }}>
                {submitting ? 'Creating...' : 'Create Packaging'}
              </button>
            </div>
          </div>
        </div>
