...
export function EditBOMModal({ bom, products, packaging, suppliers, onClose }: { bom: any; products: any[]; packaging: any[]; suppliers: any[]; onClose: () => void }) {
  ...
  async function save() {
    ...
    try {
      const res = await fetch(`/api/bom/${bom.id}`, {
        ...
        body: JSON.stringify({ ...form, qty_per_unit: parseFloat(String(form.qty_per_unit)) }),
      })
    ...
    >
      ...
      >
        <div style={{ padding: '20px 40px', borderBottom: '1px solid #D4D0C7', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
          <div>
            <div style={{ ...labelStyle, marginBottom: '2px' }}>BOM line</div>
            <h2 style={{ fontSize: '22px', fontWeight: 500, margin: 0 }}>Edit BOM line</h2>
          </div>
        ...
        <div style={{ padding: '24px 40px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', flex: 1 }}>
          ...
          <Field label="Qty per FG unit *">
            <input type="number" step="0.0001" value={form.qty_per_unit} onChange={e => update('qty_per_unit', e.target.value)} style={inputStyle} placeholder="e.g. 1 or 0.0833" />
          </Field>
          ...
          <div style={{ ...labelStyle, marginTop: '8px', marginBottom: 0 }}>
            Type auto-derived from packaging (currently: {bom.type || '—'})
          </div>
        ...
        <div style={{ padding: '16px 40px', borderTop: '1px solid #D4D0C7', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button onClick={save} disabled={submitting || deleting || !form.product_sku || !form.packaging_code || !form.qty_per_unit} style={{ padding: '8px 18px', borderRadius: '4px', fontSize: '13px', backgroundColor: '#1A1A1A', color: '#FAFAF7', border: 'none', cursor: 'pointer', opacity: (submitting || deleting || !form.product_sku || !form.packaging_code || !form.qty_per_unit) ? 0.5 : 1 }}>
